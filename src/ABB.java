...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
       ...
       recortarRec(raiz, chaveDeOnde, chaveAteOnde, resultado);
    return resultado;

	}
    private No<K,V> recortarRec(No<K,V> no, K ini, K fim, Lista<V> res){
        if(no==null){System.err.println("erro!!!!");}
       if (comparador.compare(no.getChave(),ini) <0){
       res.inserir(no.getItem());
       return recortarRec(no.getDireita(),ini,fim,res);
       }
         if (comparador.compare(no.getChave(),ini) >0){
       res.inserir(no.getItem());
       return recortarRec(no.getEsquerda(),ini,fim,res);
       }
       no.setEsquerda(recortarRec(no.getEsquerda(), ini, fim, res));
  no.setDireita(recortarRec(no.getDireita(), ini, fim, res));
